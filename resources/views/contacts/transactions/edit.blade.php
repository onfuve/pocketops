...
$oldCounterpartyId = old('counterparty_contact_id', $transaction->counterparty_contact_id);
$oldCounterpartyName = $oldCounterpartyId ? ($transaction->relationLoaded('counterpartyContact') && $transaction->counterpartyContact ? $transaction->counterpartyContact->name : (\App\Models\Contact::find($oldCounterpartyId)?->name ?? '')) : '';
$paymentType = old('payment_type', $transaction->counterparty_contact_id ? 'contact' : 'bank');
@endphp
@extends('layouts.app')

@section('title', 'ویرایش تراکنش — ' . $contact->name)
...
<div class="receive-pay-page font-vazir">
    <div class="mb-6 flex flex-wrap items-center gap-3">
        <a href="{{ route('transactions.contact-detail', $contact) }}" class="btn btn-secondary">← بازگشت به تراکنش‌های مخاطب</a>
        <h1 class="text-xl font-bold text-stone-800 sm:text-2xl">ویرایش تراکنش</h1>
    </div>
    ...
    <div class="card">
        ...
        <form method="post" action="{{ route('contacts.transactions.update', [$contact, $transaction]) }}" id="receive-pay-form">
            @csrf
            @method('PUT')
            <div class="form-group">
                <label>نوع <span style="color:#b91c1c;">*</span></label>
                <div class="payment-method type-choice">
                    <input type="radio" name="type" id="type_receive" value="receive" {{ old('type', $transaction->type) === 'receive' ? 'checked' : '' }}>
                    <label for="type_receive">دریافت از این مخاطب</label>
                    <input type="radio" name="type" id="type_pay" value="pay" {{ old('type', $transaction->type) === 'pay' ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <div class="flex gap-2 flex-wrap items-center">
                    <input type="text" name="paid_at" id="paid_at" value="{{ old('paid_at', FormatHelper::shamsi($transaction->paid_at)) }}" placeholder="۱۴۰۳/۱۱/۱۳" required
            ...
            <div class="form-group">
                ...
                <div class="payment-method">
                    <input type="radio" name="payment_type" id="pay_type_bank" value="bank" {{ $paymentType === 'bank' ? 'checked' : '' }}>
                    <label for="pay_type_bank">حساب بانکی</label>
                    <input type="radio" name="payment_type" id="pay_type_contact" value="contact" {{ $paymentType === 'contact' ? 'checked' : '' }}>
                    <label for="pay_type_contact">از طریق مخاطب دیگر</label>
                </div>
                <div id="panel-bank" class="payment-panel {{ $paymentType === 'bank' ? 'active' : '' }}">
                    <select name="payment_option_id" id="payment_option_id">
                        <option value="">انتخاب حساب بانکی</option>
                        @foreach($paymentOptions as $opt)
                            <option value="{{ $opt->id }}" {{ old('payment_option_id', $transaction->payment_option_id) == $opt->id ? 'selected' : '' }}>{{ $opt->label ?: ($opt->holder_name ?? $opt->bank_name ?? '—') }} @if($opt->account_number)({{ $opt->account_number }})@endif</option>
                        @endforeach
                    </select>
                ...
                <div id="panel-contact" class="payment-panel {{ $paymentType === 'contact' ? 'active' : '' }}">
            ...
            <div class="form-group">
                <label>یادداشت</label>
                <textarea name="notes" rows="2" placeholder="اختیاری">{{ old('notes', $transaction->notes) }}</textarea>
            </div>
