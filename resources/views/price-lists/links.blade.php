...
@section('title', 'لینک اشتراک — ' . $priceList->name . ' — ' . config('app.name'))

@push('styles')
<style>
.ds-page .ds-page-title-icon { background: #dbeafe; color: #1d4ed8; border-color: #93c5fd; }
.pl-links-card { max-width: 36rem; }
.pl-links-card input { font-family: monospace; font-size: 0.875rem; }
.pl-links-card .copy-btn { margin-top: 0.5rem; }
</style>
@endpush
...
<div class="ds-page">
    <div class="ds-page-header">
        <div>
            ...
            <p class="ds-page-subtitle">{{ $priceList->name }}</p>
        </div>
    ...
    <div class="ds-form-card pl-links-card" style="margin-top: 1.5rem;">
        ...
        <div>
            <label for="public-url" class="ds-label">آدرس</label>
            <input type="text" id="public-url" value="{{ $priceList->public_url }}" readonly class="ds-input" onclick="this.select();">
